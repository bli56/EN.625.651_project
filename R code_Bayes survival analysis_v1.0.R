...
library(writexl)
library(readxl)
data <- read_xlsx(path = "D:\\Baini_files\\ZM\\Studies\\CB03-154-201\\Sample size\\Data\\Eff_die3.xlsx")
data$armfl <- ifelse(data$Study_Arm == "Active", 1, ifelse(data$Study_Arm == "Placebo",  0, 999))

stratum_col <- data.frame(subject_id = unique(data$subject_id), stratum = sample(1:2, length(unique(data$subject_id)), replace = T))
data <- merge(data, stratum_col)

historical <- list(list(time = data$deathD,
                        event = data$deathFL,
                        X = as.matrix(data[, "armfl"]),
                        S = data$stratum))
